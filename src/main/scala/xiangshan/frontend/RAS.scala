...
class RAS extends BasePredictor
{
    ...
    if(EnableCommit){
        ...
        val retMissPred = copy_valid && io.recover.bits.pd.isRet
        val recoverSp = io.recover.bits.bpuMeta.rasSp
        val recoverTopAddr = io.recover.bits.bpuMeta.rasToqAddr
        val recoverTopCtrl = io.recover.bits.bpuMeta.rasTopCtr
        spec_ras.copy_valid := copy_valid
        spec_ras.copy_in_mem := DontCare

        spec_ras.copy_in_sp  := Mux(retMissPred && recoverTopCtrl === 1.U ,recoverSp - 1.U,recoverSp)
        spec_ras.copy_in_top.retAddr := recoverTopAddr
        spec_ras.copy_in_top.ctr     := Mux(!retMissPred , recoverTopCtrl, Mux(recoverTopCtrl === 1.U,recoverTopCtrl,  recoverTopCtrl - 1.U))

        io.meta.rasSp := spec_ras.copy_out_sp
        io.meta.rasTopCtr := spec_ras.copy_out_top.ctr
        io.meta.rasToqAddr := spec_ras.copy_out_top.retAddr
    ...
    if (BPUDebug && debug) {
        ...
        XSDebug("copyValid:%d copyNext:%d recover(SP:%d retAddr:%x ctr:%d) \n",copy_valid,copy_next,io.recover.bits.bpuMeta.rasSp,io.recover.bits.bpuMeta.rasToqAddr,io.recover.bits.bpuMeta.rasTopCtr)
    }
